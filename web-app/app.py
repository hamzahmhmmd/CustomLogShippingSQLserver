...
conn_str2 = (f'Driver={driver};Server={server},{port};Database=rahasia;UID={user};PWD={password};Trusted_Connection=no;')
...
with st.sidebar:
        ...
        with st.form(key='newbackup', clear_on_submit=True):
            ...
            if submit:
                ...
                with cnxn:
                    ...
                    ''')
                os.system(f'''
                /opt/mssql-tools/bin/sqlcmd -S {server}{pass_login}-Q "EXEC dbo.PMAG_Backup @dbname = N'{db}', @type = 'bak', @init = 1;"
                ''')
                # success
    ...
    with st.expander("Trigger Backup and Delete", expanded=True):
        ...
        with st.form(key='lsnow', clear_on_submit=True):
            ...
            if submit:
                os.system(f'''
                    /opt/mssql-tools/bin/sqlcmd -S {server}{pass_login}-Q "EXEC dbo.PMAG_Backup @dbname = N'{db}', @type = '{tipe}';"
                ''')
                # st.write(f'''/opt/mssql-tools/bin/sqlcmd -S {server}{windows_login}-Q "EXEC dbo.PMAG_Backup @dbname = N'{db}', @type = '{tipe}';"''')
                st.success('backup has been started')
        ...
        with st.form(key='clear', clear_on_submit=True):
            ...
            if submit:
                ...
                os.system(f'''
                /opt/mssql-tools/bin/sqlcmd -S {server}{pass_login} -Q "EXEC dbo.PMAG_CleanupHistory @dbname = N'{db}', @DaysOld = {dayold};"
                ''')
                if delete_trn:
                    try:
